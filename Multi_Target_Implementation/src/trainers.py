...
class Trainer:
    ...
    def loss_bce(self, seq_out, pos_ids, neg_ids):
    ...
    def loss_ce(self, seq_out, pos_ids):
        all_items_emb = self.model.item_embeddings.weight
        logits = torch.matmul(seq_out, all_items_emb.transpose(0, 1))
    ...
    def calculate_rec_loss(self, sequence_output, user_ids, input_ids, target_pos, target_neg):
        item_emb = self.model.item_embeddings.weight
        input_emb = self.model.item_embeddings(input_ids)
        pos_emb = self.model.item_embeddings(target_pos)
        neg_emb = self.model.item_embeddings(target_neg)
        pos_score = torch.sum(input_emb * pos_emb, dim=-1)
        neg_score = torch.sum(input_emb * neg_emb, dim=-1)
        ...
        if train:
            ...
            for i, rec_batch, in rec_cf_data_iter:
                ...
                if self.args.loss_type == 'BCE':
                    rec_loss = self.loss_bce(sequence_output, target_pos, target_neg)
                else:
                    rec_loss = self.loss_ce(sequence_output, target_pos)
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    if hasattr(self.model, 'transformer_encoder'):
                        ...
                        output = self.model(input_ids)
                        if isinstance(output, tuple):
                            recommend_output = output[0]
                        else:
                            recommend_output = output
                    ...
                    mask = self.args.train_matrix[batch_user_index].toarray()>0
