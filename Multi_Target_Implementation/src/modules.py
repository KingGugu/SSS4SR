...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, perturbed=None, output_all_encoded_layers=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask)

            if perturbed:
                random_noise = torch.rand_like(hidden_states)
                hidden_states += torch.sign(hidden_states) * F.normalize(random_noise, dim=-1) * 0.1
