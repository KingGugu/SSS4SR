...
class RecDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type='train'):
        ...
        self.augment_type=args.augment_type.lower() if hasattr(args, 'augment_type') else 'prefix'
        self.slide_window_size = getattr(args, 'slide_window_size', self.max_len)
        ...
        if self.data_type=='train':
            for user, seq in enumerate(user_seq):
                ...
                if self.augment_type == 'prefix':
                    sub_seqs = [input_ids[:i + 1] for i in range(len(input_ids))]
                elif self.augment_type == 'suffix':
                    sub_seqs = [input_ids[-i - 1:] for i in range(len(input_ids))]
                elif self.augment_type == 'slide':
                    sub_seqs = self._generate_slide_subseqs(input_ids)
                elif self.augment_type == 'ori':
                    sub_seqs= [input_ids]
                elif self.augment_type == 'slide_plus_ori':
                    slide_subs = self._generate_slide_subseqs(input_ids)
                    sub_seqs = slide_subs + [input_ids]
                else:
                    raise ValueError(
                        f"Unsupported seq_split_type: {self.augment_type}, choose from ['prefix', 'suffix', 'slide',"
                        f"'ori', 'slide_plus_ori']")

                self.user_seq.extend(sub_seqs)
                self.user_ids.extend([user] * len(sub_seqs))
    ...
    def _generate_slide_subseqs(self, seq):
        """generate slide subSeq"""
        sub_seqs = []
        seq_len = len(seq)
